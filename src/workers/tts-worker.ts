...
async function handleSynthesize(request: SynthesizeRequest) {
    const { chunkText, chunkHash, speedWpm, useWebGPU } = request;

    // Synthesize using the engine
    const { audioData, sampleRate, durationSec, wavBuffer: preGeneratedWav } = await ttsEngine.synthesize(chunkText, speedWpm, useWebGPU ?? false);
